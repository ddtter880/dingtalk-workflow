/**
 * 在线表格数据接口
 * GET /api/sheet?sheetId=xxx&range=A1:Z100
 * POST /api/sheet (追加行)
 * PUT /api/sheet (更新区域)
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    switch (req.method) {
      case 'GET': {
        // 读取表格数据
        const { sheetId, range } = req.query as { sheetId?: string; range?: string }

        if (!sheetId || !range) {
          return res.status(400).json({ error: '缺少sheetId或range参数' })
        }

        const data = await getSpreadsheetRange(SPREADSHEET_ID, sheetId, range)
      ...
      case 'POST': {
        ...
        const data = await appendSpreadsheetRows(SPREADSHEET_ID, sheetId, values)
        return res.status(200).json({ success: true, data })
      }
      ...
      case 'PUT': {
        ...
        const data = await updateSpreadsheetRange(SPREADSHEET_ID, sheetId, range, values)
        return res.status(200).json({ success: true, data })
      }
